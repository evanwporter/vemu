...
static uint16_t rtl_read16(int offset) {
    return static_cast<uint16_t>(rtl_registers[offset / 4] >> ((offset % 4) * 8));
}

static uint32_t rtl_read32(int offset) {
    return rtl_registers[offset / 4];
}

static void sync_registers_from_rtl() {
    const uint16_t status = ioreg.dispstat.w & 7;
    ioreg.dispcnt.w = rtl_read16(REG_DISPCNT);
    ioreg.dispstat.w = (rtl_read16(REG_DISPSTAT) & ~7u) | status;
    for (int i = 0; i < 4; ++i) {
        ioreg.bgcnt[i].w = rtl_read16(REG_BG0CNT + i * 2);
        ioreg.bg_text[i].x.w = rtl_read16(REG_BG0HOFS + i * 4);
        ioreg.bg_text[i].y.w = rtl_read16(REG_BG0VOFS + i * 4);
    }
    for (int i = 0; i < 2; ++i) {
        const int base = REG_BG2PA + i * 0x10;
        ioreg.bg_affine[i].pa.w = rtl_read16(base);
        ioreg.bg_affine[i].pb.w = rtl_read16(base + 2);
        ioreg.bg_affine[i].pc.w = rtl_read16(base + 4);
        ioreg.bg_affine[i].pd.w = rtl_read16(base + 6);
        ioreg.bg_affine[i].x0.dw = rtl_read32(base + 8);
        ioreg.bg_affine[i].y0.dw = rtl_read32(base + 12);
    }
    for (int i = 0; i < 2; ++i) {
        ioreg.winh[i].w = rtl_read16(REG_WIN0H + i * 2);
        ioreg.winv[i].w = rtl_read16(REG_WIN0V + i * 2);
    }
    ioreg.winin.w = rtl_read16(REG_WININ);
    ioreg.winout.w = rtl_read16(REG_WINOUT);
    ioreg.mosaic.w = rtl_read16(REG_MOSAIC);
    ioreg.bldcnt.w = rtl_read16(REG_BLDCNT);
    ioreg.bldalpha.w = rtl_read16(REG_BLDALPHA);
    ioreg.bldy.w = rtl_read16(REG_BLDY);
}
