...
`define DISPLAY_DECODED_MULT(word) \
  begin \
    $display("---- DECODED WORD (MULT) ----"); \
    $display("IR           = 0x%08x", (word).IR); \
    $display("instr_type   = %s", (word).instr_type.name()); \
    $display("cond pass    = %0d", (word).condition_pass); \
    $display("Rn (accum)   = R%0d", (word).Rn); \
    $display("Rd (dest)    = R%0d", (word).Rd); \
    $display("Rm (operand) = R%0d", (word).Rm); \
    $display("Rs (operand) = R%0d", (word).Rs); \
    $display("opcode       = %s", (word).immediate.mul.opcode.name()); \
    $display("--------------------------------"); \
    $fflush(); \
  end
