...
inline void print_debug_mismatch(const std::optional<TraceRow>& last_good,
...
inline NbaOracle CreateNbaOracle(const fs::path& rom_path,
    const std::optional<fs::path>& bios_path = std::nullopt) {
    NbaOracle oracle;
...
inline TraceRow capture_nba_state(nba::core::Core* core, size_t step_index) {
    ...
    row.r = { state.r0, state.r1, state.r2, state.r3, state.r4, state.r5,
        state.r6, state.r7, state.r8, state.r9, state.r10, state.r11,
...
inline void compare_states(const TraceRow& expected,
    const TraceRow& actual,
