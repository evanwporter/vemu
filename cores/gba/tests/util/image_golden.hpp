...
inline void expect_golden_image(const GbaFramebuffer& actual,
    const std::filesystem::path& golden_path) {
    const auto actual_path = std::filesystem::current_path()
        / "golden-actual" / (golden_path.stem().string() + ".actual.ppm");
    std::filesystem::create_directories(actual_path.parent_path());
    vemu::write_ppm(actual, SCREEN_WIDTH, SCREEN_HEIGHT, actual_path,
        vemu::PixelFormat::ABGR8888);

    if (test_config().update) {
        std::filesystem::create_directories(golden_path.parent_path());
        std::filesystem::copy_file(actual_path, golden_path,
            std::filesystem::copy_options::overwrite_existing);
    }

    std::size_t width = 0;
    std::size_t height = 0;
    const auto expected = vemu::read_ppm(golden_path, width, height,
        vemu::PixelFormat::ABGR8888);
